...
class PDFMerger:
    def __init__(self, input_folder, output_folder, reference_doc=None, edi_file=None, 
        ...
        # ADD THIS LINE: Global tracking for Bill clients to prevent duplicates
        self.processed_bill_clients = set()
    ...
    def process_multi_client_document(self, pdf_path, doc_type):
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                # DEBUG: Log text sample for Advice documents
                if doc_type == 'Advice of Arrivals' and page_num < 3:  # First 3 pages only
                    self.logger.info(f"DEBUG Advice page {page_num + 1} text sample: {text[:300]}...")
                
                # Look for reference patterns in the text
                # ... (your new pattern code from Step 2)
                # Look for reference patterns in the text
                # ENHANCED: Target specific "Consignee Reference" labels for Advice documents
                if doc_type == 'Advice of Arrivals':
                    # For Advice documents, look specifically for "Consignee Reference" labels
                    ref_patterns = [
                        r'Consignee\s+Reference[:\s]*[#]?\s*(\d{3}[-/]\d{3}[-/]\d{3}(?:[-/]\d+)?)',
                        r'Consignee\s+Ref[:\s]*[#]?\s*(\d{3}[-/]\d{3}[-/]\d{3}(?:[-/]\d+)?)',
                        r'Consignee[:\s]+(\d{3}[-/]\d{3}[-/]\d{3}(?:[-/]\d+)?)',
                        r'(\d{3}[-/]\d{3}[-/]\d{3})',  # Fallback generic pattern
                    ]
                else:
                    # For other document types, use generic patterns
                    ref_patterns = [
                        r'(\d{3}[-/]\d{3}[-/]\d{3})',  # 000-000-000 or 000/000/000
                        r'(\d{3}\s+\d{3}\s+\d{3})',   # 000 000 000
                    ]
                
                # Track if we already found a client on this page
                page_client_found = False
                
                for pattern in ref_patterns:
                    if page_client_found:  # Skip remaining patterns if we already found a client
                        break
                        
                    matches = re.findall(pattern, text, re.IGNORECASE)
                    
                    # DEBUG: Log what we found
                    if matches and doc_type == 'Advice of Arrivals':
                        self.logger.info(f"DEBUG Found matches on page {page_num + 1} with pattern '{pattern}': {matches}")
                    ...
                    for match in matches:
                        ...
                        if doc_type == 'Advice of Arrivals':
                            self.logger.info(f"DEBUG Trying to match ref_base '{ref_base}' against EDI manifest")
                        ...
                        if matched_ref:
                            ...
                            # ENHANCED: For Bill documents, check global tracking to prevent cross-document duplicates
                            if doc_type == 'Bill of Lading':
                                if matched_ref not in self.processed_bill_clients:
                                    ...
                                    # Mark this client as processed for ALL Bill documents globally
                                    self.processed_bill_clients.add(matched_ref)
                                    page_client_found = True
                                    break
                                else:
                                    self.logger.debug(f"{doc_type}: Skipping {matched_ref} - already has Bill page from another document")
                                    page_client_found = True
                                    break
                            else:
                                # For non-Bill documents (Advice, Customer docs), use the original logic
        ...
        try:
            ...
            if ref_match:
                ...
                if file_ref in self.manifest:
                    ...
                    # DEBUG: Add enhanced logging
                    self.logger.info(f"DEBUG: About to add {len(doc)} pages for customer doc {file_ref}")
                    self.logger.info(f"DEBUG: Current client {file_ref} has {len(self.clients[file_ref]['pages'])} pages before adding")
                    ...
                    for page_num in range(len(doc)):
                        ...
                        self.logger.debug(f"DEBUG: Added page {page_num} for {file_ref}")
                    
                    # DEBUG: Verify pages were added
                    self.logger.info(f"DEBUG: Finished adding pages. Client {file_ref} now has {len(self.clients[file_ref]['pages'])} total pages")
            ...
            for edi_ref in self.manifest.keys():
                if edi_ref in text or edi_ref.replace('/', '-') in text:
                    ...
                    # DEBUG: Add enhanced logging for content match path too
                    self.logger.info(f"DEBUG: About to add {len(doc)} pages for customer doc {edi_ref} (content match)")
                    self.logger.info(f"DEBUG: Current client {edi_ref} has {len(self.clients[edi_ref]['pages'])} pages before adding")
                    ...
                    for page_num in range(len(doc)):
                        ...
                        self.logger.debug(f"DEBUG: Added page {page_num} for {edi_ref}")
                    
                    # DEBUG: Verify pages were added
                    self.logger.info(f"DEBUG: Finished adding pages. Client {edi_ref} now has {len(self.clients[edi_ref]['pages'])} total pages")
    ...
    def process_all_documents(self):
        ...
        for pdf_path in pdf_files:
            ...
            if 'advice' in filename_lower:
                self.logger.info(f"🔍 DEBUG: Found advice file: '{filename}'")
                self.logger.info(f"🔍 DEBUG: Lowercase: '{filename_lower}'")
                self.logger.info(f"🔍 DEBUG: Contains 'advice of arrival': {'advice of arrival' in filename_lower}")
